...
my $minq = 0;
my $minlen = 0;
my $minlen_terminal = 10;
my $minlen_internal = 15;
my $minave = 33;
my $winsize = 1;
my $outbase;
...
my $primposfile1 = shift;
my $infile1 = shift;
my $primposfile2 = shift;
my $infile2 = shift;
...
unless(length($primposfile1) > 0 && length($infile1) > 0) {
    die $help;
}

if(length($primposfile2) > 0 && length($infile2) > 0) {
    $paired = 1;
}
...
my @notfound;
unless(-e $infile1) { push(@notfound, $infile1); }
unless(-e $primposfile1) { push(@notfound, $primposfile1); }
unless(!$paired || -e $infile2) { push(@notfound, $infile2); }
unless(!$paired || -e $primposfile2) { push(@notfound, $primposfile2); }
if(scalar(@notfound) > 0) {
    die "File(s) not found:\n".join("\n", @notfound)."\n";
}

if($primposfile1 =~ /\.gz$/) {
    open(INP1, "gunzip -c $primposfile1 2>/dev/null |");
} else {
    open(INP1, $primposfile1) or die "Unable to open file $primposfile1\n";
}
if($infile1 =~ /\.gz$/) {
...
if($paired) {
    if($primposfile2 =~ /\.gz$/) {
	open(INP2, "gunzip -c $primposfile2 2>/dev/null |");
    } else {
	open(INP2, $primposfile2) or die "Unable to open file $primposfile2\n";
    } 
    if($infile2 =~ /\.gz$/) {
...
print STDERR "Reading primer hits file: $primposfile1\n";
while(<INP1>) {
    chomp;
    my ($id, $plist) = split(/\t/);
    $idppos1{$id} = $plist;
...
if($paired) {
    print STDERR "Reading primer hits file: $primposfile2\n";
    while(<INP2>) {
	chomp;
	my ($id, $plist) = split(/\t/);
	$idppos2{$id} = $plist;
...
if($count_tot > 0) {
	...
	printf OUTSUM "Bases primer-trimmed 1 (ave) : %d (%.1f)\n", $count_bases_qp{1}, 2 * $count_bases_qp{1} / $count_tot;
	...
	printf OUTSUM "Bases primer-trimmed 2 (ave) : %d (%.1f)\n", $count_bases_qp{2}, 2 * $count_bases_qp{2} / $count_tot;
	...
	printf OUTSUM "Bases primer-trimmed (ave)   : %d (%.1f)\n", $count_bases_qp{1}, $count_bases_qp{1} / $count_tot;
    }
    close(OUTSUM);
}
...
sub qtrim($$$$) {
    ...
    if(defined($plist)) {
	foreach my $p (split(/,/, $plist)) {
	    my ($s, $e) = split(/\-/, $p);
	    if($e-$s+1 >= $minlen_terminal) {
		if($s-1 <= 20 && $e > $minpos) {  # trim more from beginning
		    $minpos = $e;
		}
		if($e >= $n-20 && $s-2 < $maxpos) { # trim more from end
		    $maxpos = $s-2;
		}
	    }
	    if($e-$s+1 >= $minlen_internal) {
		if($s-1 > 20 || $e < $n-20) {  
		    $minpos = 1; # eliminate read by setting minpos > maxpos
		    $maxpos = 0;
		}
	    }
	}
    ...
    $count_bases_qp{$whichread} += $maxpos-$minpos+1;
