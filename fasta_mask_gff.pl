...
unless($infile && $gfffile) {
    die "Usage: $0 [FASTA file (or .gz)] [GFF file]\nOutput FASTA with all regions specified in GFF file masked\n";
}
...
while(<IN>) {
    chomp;
    my ($ctg, $source, $type, $start, $end) = split(/\t/);
    $masknum++;
    $maskctg{$masknum} = $ctg;
