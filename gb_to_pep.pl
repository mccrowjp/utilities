...
while(my $objseq = $objseqio->next_seq) {
	...
	my ($pepseq) = $objfeat->get_tag_values('translation');	    
	my ($locus, @others);

	unless(length($pepseq) > 0) {
	    print STDERR "No translation found for CDS on $acc\n";
	}
	...
	unless(length($locus) > 0) {
	    print STDERR "No gene/locus_tag found for CDS on $acc\n";
	}

	if(length($locus) > 0 && length($pepseq) > 0) {
	    printf ">%s %s %s\n%s\n", $locus, $protid, $prod_ann, $pepseq;
	}
    }
}
